all:
	...
	# build. 32768 sectors (16MiB) instead of the old 2880 (1.44MiB) to give
	# the fs.c region (see fs.h FS_END_LBA) headroom. An image built before
	# this change is still only 1.44MiB on disk and needs `make distclean
	# && make all` to pick up the new size, same as any other on-disk-layout
	# change documented in README.md's "Orçamento de boot".
